 ...
 */
void FileCompressor::do_Fano_Algorithm(std::optional<std::span<std::pair<uint8_t, size_t>>> vec, uint32_t vec_sum) {
    ...
    uint32_t sum_prev = 0, sum_cur = 0, i = 0;

    while (i < occur.size() && sum_cur + occur[i].second <= vec_sum / 2) {
        sum_prev = sum_cur;
        sum_cur += occur[i++].second;
    }
    uint32_t left_sum = sum_cur;
    uint32_t right_sum = vec_sum - left_sum;
...
void FileCompressor::count_occurances(const std::string& filename) {
    ...
    while (file.read(reinterpret_cast<char*>(&character), 1)) {
        ...
        if (it != occurrences.end()) {
            ...
            occurrences.push_back({ character, 1 });
        }
        occur_sum++;
    }
}
...
void FileCompressor::write_encoded_data(std::fstream& out, const std::string& input_file) const {
    ...
    while (in.read(reinterpret_cast<char*>(&ch), 1)) {
        data.push_back(ch);
        totalBits += static_cast<uint32_t>(codes.at(ch).size());
    }
 ...
 */
void FileCompressor::load_archived(const std::string& filename) {
...
void FileCompressor::decode_bitstream(std::fstream& in, std::fstream& out) {
    ...
    while (bitsRead < totalBits) {
        ...
        if (it != code_to_symbol.end()) {
            out.write(reinterpret_cast<const char*>(&it->second), 1);
            bitsBuffer.clear();
        }
    }
}
